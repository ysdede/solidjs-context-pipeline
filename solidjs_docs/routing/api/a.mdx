...
source_path: solid-docs\src\routes\solid-router\reference\components\a.mdx
domain: routing
extracted_at: "2026-02-01T15:19:27.027Z"
---
...
`` supports relative and base paths. `<a />` doesn't. But `<a />` gets augmented
when JS is present via a top-level listener to the DOM, so you get the
soft-navigation experience nonetheless.

The `` supports the [``](/solid-router/reference/components/router) base property (`<Router base="/subpath">`) and prepend it to the received `href` automatically and the `<a />`does not.
The same happens with relative paths passed to ``.
...
To prevent, both `` and `<a />` tags from soft navigating when JavaScript is present, pass a `target="_self"` attribute.
