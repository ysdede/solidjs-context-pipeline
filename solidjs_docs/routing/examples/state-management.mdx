...
source_path: solid-docs\src\routes\guides\state-management.mdx
domain: routing
...
2. **View (`{count()}`)**: The _visual representation_ of the state to the user.
...
function Counter() {
	...
	return (
		<>
			Current count: {count()}
			Doubled count: {doubleCount()} // Display the doubled count
			<button onClick={increment}>Increment</button>
		</>
	);
}
